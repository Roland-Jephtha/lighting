...
(function() {
  ...
  function closeWalletModal() {
    ...
    // our HTML toggles the "active" class when showing the modal
  ...
  async function connectMetaMask() {
    // Implement MetaMask detection and connect flow similar to working example
    function getMetaMaskProvider() {
      ...
      if (window.ethereum && Array.isArray(window.ethereum.providers) && window.ethereum.providers.length) {
        var mm = window.ethereum.providers.find(function(p) { return p && p.isMetaMask && !p.isBraveWallet; });
        if (mm) return mm;
      }
      // Direct MetaMask
    ...
    var provider = getMetaMaskProvider();

    if (!provider) {
      // show QR/install flow
    ...
    try {
      var requestFn = (typeof provider.request === 'function') ? provider.request.bind(provider) : null;

      // Prefer eth_requestAccounts; fallback to enable()
      var accounts = null;
      if (requestFn) {
        accounts = await requestFn({ method: 'eth_requestAccounts' });
      ...
      if (err && err.code === -32002) {
        alert('A MetaMask connection request is already pending. Please check the MetaMask extension or toolbar and approve the request.');
        return;
      }
      if (err && err.code === 4001) return; // user rejected

      // Detect provider hijack / missing request function
      if (err && err.message && (err.message.includes('request is not') || err.message.includes('Cannot read properties of undefined'))) {
        // Show QR/troubleshoot flow
        showQrPanel('MetaMask');
        alert('Provider appears to be another wallet or hijacked. Disable other wallet extensions and try again.');
        return;
      }

      alert('MetaMask connection failed: ' + (err && (err.message || err.toString()) || 'Unknown error'));
    }
  }
